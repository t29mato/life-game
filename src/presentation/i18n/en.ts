...
export const EN = {
  ...
  card: {
    ...
    passed: (title: string, times: number): string => `Passed ${title}${times > 1 ? ` ×${times}` : ''}`,
    passedWithMoney: (title: string, times: number, money: string): string =>
      `Passed ${title}${times > 1 ? ` ×${times}` : ''} · ${money}`,
  },
