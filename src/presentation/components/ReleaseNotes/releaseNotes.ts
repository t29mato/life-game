 ...
 *
 * **One name per thing, and the name follows the object.** The randomiser is
 * *the wheel*, and you *spin* it — everywhere on this page and everywhere on
 * screen.
 *
 * This file has now been swept twice, in opposite directions, and both sweeps
 * obeyed the same rule. The first (issue D4) rewrote a record that said
 * "wheel" and "spin" into "die" and "roll", because the thing on screen was a
 * die and a player reading their own history should not find a second name
 * for the object in front of them. It ended with an instruction not to
 * reintroduce "wheel" or "spin" anywhere. That instruction is deliberately
 * withdrawn here, not forgotten: the object itself went back to being a
 * wheel — a physical spinner, ticker clicking past the pegs as it slows,
 * because a die is already decided the moment it leaves your hand and gives
 * a table nothing to watch — so the record follows it back.
 *
 * The rule was never "say die". It was: the notes call the randomiser
 * whatever is on the table, in one voice, front to back. Renaming the whole
 * file rather than half of it is what makes that honest — the object changed,
 * not the history of what each release did to it.
 *
 * So, until the object changes again: no "die", "dice", "roll", "rolled" or
 * "rolling" for the randomiser, here or anywhere a player can read it. And if
 * it does change again, sweep this file with it in one pass and rewrite this
 * paragraph — do not leave the next reader two commits arguing and no reason.
 * `releaseNotes.test.ts` holds that rule to the notes themselves.
 */
...
export const RELEASE_NOTES: readonly ReleaseNote[] = [
  {
    ...
    whatsNew: [
      ...
      'A key to the board, dealt once before your very first spin and kept in the Handbook after that: what the coin means, what the arrows mean, what the red-and-white stripe means, and what the colour under a tile is telling you.',
    ...
    changes: [
      ...
      "Payday itself always shows a card now, even on a flat salary with nothing to spin for — the money still changed, so it still gets a moment on screen.",
    ...
    fixes: [
      ...
      'Fixed the Handbook: it still said a fork asks you to pick a road and then spin again. It has not for a long time — the first spin picks the road for you, 1 to 3 one way and 4 to 6 the other, and the second is the distance.',
  ...
  },
  {
    ...
    whatsNew: [],
    changes: [
      "Most stops aren't stops any more. Tuition, a promotion review, a new career, a wedding, a new baby — the news still happens exactly the same, but a big spin no longer gets cut short landing on the tile that delivers it. Only a handful of tiles that ask a real question — buying a house, retiring early — still halt your turn.",
      "Early retirement spells out the bonus itself, not just what it nets against the stake, so a modest spin doesn't read as a loss when the fund you just locked in is a real one.",
  ...
  },
  {
    ...
    whatsNew: [],
    changes: [
      "Forks no longer ask you to choose a road before you spin — the wheel decides which road you take, low half one way and high half the other. Nobody who has played the game before gets a head start on the players who haven't.",
  ...
  },
  {
    ...
    changes: [
      "No more spoilers: the board itself used to jump to your landing tile the instant you pressed the wheel, well before it had visibly finished turning. Now it waits for the wheel, same as the result card already did.",
  ...
  },
  {
    ...
    changes: [
      "Fewer back-to-back forced stops. A few spots on the board used to chain two mandatory stops right next to each other — right after graduation, and right after taking the Job-Hopper route — so the spin straight after barely mattered. Both now have a beat in between, so every spin still counts for something.",
  ...
  },
  {
    ...
    whatsNew: [],
    changes: [
      "The wheel is turned from its own middle now — tap or click the wheel itself to spin it, the way you would flick one with your hand. The separate button beside it is gone, so the wheel gets to be bigger for it. Space still spins it exactly as before.",
      "A spin that has nothing to do with where anyone's token sits — tuition, a career choice, a promotion review, a marriage proposal — now lands front and centre in its own window over the middle of the screen, instead of down in the sidebar. An ordinary move spin still happens right beside the board, since that one decides where you actually go.",
  ...
  },
  {
    ...
    fixes: [
      "Fixed the title screen's floating decorations — the cap and the rest — landing on top of the game's own name on a narrow phone screen.",
    ],
  },
  {
    version: 'v1.4.4',
  ...
  },
  {
    ...
    changes: [],
    fixes: [
      "Fixed the Space bar shortcut freezing the game: pressing it disabled the wheel instead of turning it, and there was no way back short of reloading. Space now presses the wheel for real.",
      "Fixed a spoiler: a wheel-decided tile (tuition, career choice, a promotion review, and the rest) could update a player's cash or job title in the sidebar while the wheel was still turning, before it had actually landed. The sidebar now waits for the wheel to settle, same as the result card already did.",
    ],
  },
  {
    version: 'v1.4.2',
  ...
  },
  {
    ...
    changes: [],
    fixes: [
      "Fixed every spin-for-it tile — tuition, career choice, a promotion review, a marriage proposal, and the rest — so pressing the wheel actually turns the one you can see, instead of the result just appearing in the result card with the wheel sitting there disabled. The card now shows what's on the line, then hands the real wheel to you (or, on a computer's turn, spins it the same way a person would) rather than deciding everything the instant the card opened.",
    ],
  },
  {
    version: 'v1.4.0',
    date: 'August 19, 2026',
    whatsNew: [
      "Career choice is on the wheel now. Job fairs, headhunters, Job-Hopper Alley, and every other career tile put two named jobs on the table and spin for which one you get, instead of you just picking whichever pays more. The card names both jobs and which half of the wheel takes you to each, before you press it. Where the tile allows it, you can still turn both down and keep the job you already have.",
      "College tuition is a spin too. Instead of one flat bill, the wheel decides what you owe — a bad spin costs more than the old flat rate, a great one can mean a full ride, and the card spells out every band before you press it.",
    ],
    changes: [
      "The desktop wheel is bigger again, and turns more slowly, so the numbers are easier to follow as they go by.",
    ],
    fixes: [],
  },
  {
  ...
  },
  {
    ...
    changes: [],
    fixes: [
      "Fixed a bad one from the last release: on a phone, the board could render far taller than the space it was given and print straight through the wheel and the player cards below it, breaking the whole screen. It now always stays sized to fit above them, like it's meant to.",
    ],
  },
  {
    version: 'v1.3.0',
    ...
    whatsNew: [
      ...
      'A new baby now gets its own spin. Land on New Baby, Twins, or Another Arrival and press the wheel for the gift envelopes, same as any other spin-for-money tile — higher is always better.',
    ],
    changes: [
      "Wedding proposals, promotions, and the household joint-account tile now wait for you to press the wheel too, the same way payday cards already did — the game no longer spins before you've seen what's on the tile.",
      "The wheel is bigger on a desktop screen, and every player's token on the board is larger too, so it's easier to see who's standing where at a glance.",
  ...
  },
  {
    ...
    date: 'August 14, 2026',
    whatsNew: [
      'A spin-for-money card now names its rate up front and waits for you to press the wheel — instead of the game spinning for you before you saw the tile. Higher is always better, and now you find that out yourself.',
    ],
    changes: [
  ...
  },
  {
    ...
    fixes: [
      ...
      'On a short game, the opening spin used to do nothing if you chose the degree lane — the pawn always stopped one tile later regardless of the number. Fixed.',
    ],
  },
  {
    version: 'v1.1.0',
