...
describe('a decision card answered by turning the die', () => {
  ...
  it('throws a die the player can watch before the Career Fair card says what they rolled', async () => {
    ...
    // `SPIN_WAIT` because the wheel takes 2.6s where the die took ~1.15s, and
    // `Spun` because that is what the label says now — both halves of this
    // session, and each one wrong without the other.
    await waitFor(() => expect(screen.getByRole('button', { name: /continue/i })).toBeInTheDocument(), SPIN_WAIT)
