...
describe('stocks catalog', () => {
  ...
  describe('findStock', () => {
    it('finds a stock by id', () => {
      const target = STOCKS[0]
      expect(target).toBeDefined()
      expect(findStock(target!.id)).toEqual(target)
    })

    it('finds the last stock in the list too', () => {
      const target = STOCKS[STOCKS.length - 1]
      expect(findStock(target!.id)).toEqual(target)
    })

    it('returns undefined for an unknown id', () => {
      expect(findStock('not-a-real-stock')).toBeUndefined()
    })
  })
})
