...
describe('france stocks catalog', () => {
  ...
  it('holds at least two safe stocks whose range barely dips under the price', () => {
    // The toll roads and the reactors: never close, never miss, never surprise.
  ...
  describe('findStock', () => {
    it('finds a stock by id in the france edition', () => {
      const target = STOCKS[0]!
      expect(findStock(target.id, EDITION_FRANCE)).toEqual(target)
    })

    it('returns undefined for an unknown id', () => {
      expect(findStock('not-a-real-stock', EDITION_FRANCE)).toBeUndefined()
    })
  })
})
