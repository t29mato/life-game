...
describe('neither opening lane is the right answer, in euros either', () => {
  ...
  it('leaves neither lane the obvious money play', () => {
    const gap = Math.abs(mean(standard.grandeEcole) - mean(standard.work))
    expect(gap / mean(standard.grandeEcole)).toBeLessThan(0.15)
  })
...
describe('insurance pays off about twice a game, in euros', () => {
