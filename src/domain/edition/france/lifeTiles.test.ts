...
describe('france life tile deck', () => {
  ...
  it('gives every tile a positive value roughly between €10k and €150k', () => {
    for (const tile of LIFE_TILE_DECK) {
  ...
  describe('findLifeTile', () => {
    it('finds a tile by id in the france edition', () => {
      const target = LIFE_TILE_DECK[0]!
      expect(findLifeTile(target.id, EDITION_FRANCE)).toEqual(target)
    })

    it('returns undefined for an unknown id', () => {
      expect(findLifeTile('not-a-real-tile', EDITION_FRANCE)).toBeUndefined()
    })
  })
})
