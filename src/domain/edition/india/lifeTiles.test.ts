...
describe('india life tile deck', () => {
  ...
  it('gives every tile a positive value roughly between ₹10 lakh and ₹1.5 crore', () => {
    for (const tile of LIFE_TILE_DECK) {
  ...
  describe('findLifeTile', () => {
    it('finds a tile by id in the india edition', () => {
      const target = LIFE_TILE_DECK[0]!
      expect(findLifeTile(target.id, EDITION_INDIA)).toEqual(target)
    })

    it('returns undefined for an unknown id', () => {
      expect(findLifeTile('not-a-real-tile', EDITION_INDIA)).toBeUndefined()
    })
  })
})
