...
/**
 * The India career pools, held to the same shape the USA pools were measured
...
describe('india careers catalog', () => {
  ...
  it('never hires anybody straight onto a fortune', () => {
    // Nobody is handed the highway dhaba on Joining Day: a fair deals the
  ...
  it('keeps the graduate pool narrow: a dependable wage, never a spectacular one', () => {
    // The placement prize is a floor, not a ceiling: a whole graduate life
  ...
  it('keeps the professions that could not possibly vary on a flat salary', () => {
    for (const id of ['career-in-hospital-surgeon', 'career-in-university-professor', 'career-in-district-collector']) {
      expect(findCareer(id, EDITION_INDIA)?.payPerPip).toBeUndefined()
    }
  })

  it('pays the trades whose weeks really do differ by the spin', () => {
    // The cart's evening crowd, the rider's surge hours, the serial's ratings.
    for (const id of ['career-in-chaat-cart-owner', 'career-in-delivery-rider', 'career-in-tv-serial-writer']) {
      expect(findCareer(id, EDITION_INDIA)?.payPerPip).toBeGreaterThan(0)
    }
  })
