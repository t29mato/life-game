...
/**
 * The India edition's balance, measured rather than inherited on trust.
 *
 * The route mirrors the USA skeleton at ×100 (`edition.test.ts` pins that
 * mechanically, houses and all) and the CPU is proven scale-invariant, so in
 * principle every measured USA property carries over with two more zeros. A
 * principle nobody measures is a hope, so this suite plays the same seeded
 * games the USA suite plays, on the India board, and asserts the same bands in
 * rupees: the even opening fork, Straight to Work as the volatile road, the
 ...
 *
 * The lead wires editions into the shipped registry; this suite registers
 * India itself, which is exactly what `registerEdition` is for. Without it,
 * `editionFor('india')` would silently fall back to the USA board — which is
 * why `playGame` also asserts the finished game's `editionId`.
 */
registerEdition(EDITION_INDIA)
...
/** Drives a complete India game, taking the option at `optionBias % length`. */
const playGame = (
  seed: number,
  playerCount: number,
  optionBias: number,
  ...
  expect(finalState.editionId).toBe('india')
...
describe('every india game reaches a conclusion', () => {
...
describe('the rupee economy stays in a playable band', () => {
  ...
  it('keeps normal comfortably profitable — the USA band, in rupees', () => {
    ...
    // every later draw the same way any other route change in this file
    // already does — re-measured at ₹7.01 crore, a hair over the old ceiling.
    expect(mean(normal.totals)).toBeGreaterThan(40_000_000)
    expect(mean(normal.totals)).toBeLessThan(71_000_000)
...
describe('neither opening lane is the right answer, in rupees either', () => {
  const COLLEGE = 'The College Route'
  ...
  const splitOf = (seeds: readonly number[], options: PlayOptions = {}): Split => {
    ...
    for (const seed of seeds) {
      ...
      finalState.players.forEach((player, seat) => {
        ...
        ;(seat === 0 ? college : work).push(total)
      })
      if (finalState.players.findIndex((p) => p.id === results.winnerId) === 0) collegeWins += 1
    }
  ...
  it.each([
    ...
    expect(sample.collegeWinRate).toBeGreaterThan(0.33)
    expect(sample.collegeWinRate).toBeLessThan(0.62)
  })
})

describe('the computer can play the india board unaided', () => {
  ...
  it('still buys houses when every rung of the ladder appreciates', () => {
