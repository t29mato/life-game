...
describe('india stocks catalog', () => {
  ...
  it('holds at least two safe stocks whose range barely dips under the price', () => {
    // The dairy cooperative and the solar parks: never close, never miss,
    // never surprise.
  ...
  describe('findStock', () => {
    it('finds a stock by id in the india edition', () => {
      const target = STOCKS[0]!
      expect(findStock(target.id, EDITION_INDIA)).toEqual(target)
    })

    it('returns undefined for an unknown id', () => {
      expect(findStock('not-a-real-stock', EDITION_INDIA)).toBeUndefined()
    })
  })
})
