...
describe('the researcher france economy stays in a playable band', () => {
  ...
  it('keeps normal comfortably profitable — the country board\'s band, in the same euros', () => {
    // Both halves of this session changed what this board measures — the
    // layoff payday moved, and a new arrival became a die — so neither
    // branch's quoted figures survived the merge. Re-measured on the merged
    // tree; the numbers in this comment are from that run.
    //
