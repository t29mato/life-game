...
describe('bolivia stocks catalog', () => {
  ...
  it('holds at least two safe stocks whose range barely dips under the price', () => {
    // The brewery and the cement works: every fiesta, every new floor.
  ...
  describe('findStock', () => {
    it('finds a stock by id in the bolivia edition', () => {
      const target = STOCKS[0]!
      expect(findStock(target.id, EDITION_BOLIVIA)).toEqual(target)
    })

    it('returns undefined for an unknown id', () => {
      expect(findStock('not-a-real-stock', EDITION_BOLIVIA)).toBeUndefined()
    })
  })
})
