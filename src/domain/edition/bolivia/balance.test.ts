...
describe('the boliviano economy stays in a playable band', () => {
  ...
  it('keeps normal comfortably profitable — the USA band, in bolivianos', () => {
    ...
    // Measured before the die: mean Bs 661k, median Bs 646k over these 60 games, and nobody
    // ever bust on normal — the USA band digit for digit, as a ×1 mirror
    // with no catalogue deviation must be. Every fork is the wheel's own call
    // now (see spin.ts), which reshuffles every later draw the same way any
    // other route change in this file already does — re-measured at
    // Bs 700.8k, a hair over the old ceiling.
...
describe('insurance pays off, rarely but really, in bolivianos', () => {
