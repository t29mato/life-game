...
/**
 * The Bolivia career pools, held to the same shape the USA pools were
 * measured at — because the shape of the two pools *is* the balance of the
 * opening fork, and this edition changes the country, never the shape.
 * Every threshold below is the USA suite's at ×1.
 */
...
describe('bolivia careers catalog', () => {
  ...
  it('never hires anybody straight onto a fortune', () => {
    // Nobody is handed the shopping arcade on their first Monday at the
    // market: a fair deals the bottom rung, and the bottom rung earns like one.
  ...
  it('keeps the graduate pool narrow: a dependable wage, never a spectacular one', () => {
    // The professional card is a floor, not a ceiling: a whole graduate life
  ...
  it('keeps the professions that could not possibly vary on a flat salary', () => {
    for (const id of ['career-bo-hospital-surgeon', 'career-bo-university-professor', 'career-bo-highway-engineer']) {
      expect(findCareer(id, EDITION_BOLIVIA)?.payPerPip).toBeUndefined()
    }
  })

  it('pays the businesses whose weeks really do differ by the spin', () => {
    // The stall's foot traffic, the fare-caller's passengers, the container's
    // reception at the market.
    for (const id of ['career-bo-anticucho-cart', 'career-bo-fare-caller', 'career-bo-container-importer']) {
      expect(findCareer(id, EDITION_BOLIVIA)?.payPerPip).toBeGreaterThan(0)
    }
  })
