...
/**
 * The Japan career pools, held to the same shape the USA pools were measured
 * at — because the shape of the two pools *is* the balance of the opening
 * fork, and this edition changes the unit and the country, never the shape.
 * Every threshold below is the USA suite's at ×100.
 */
...
describe('japan careers catalog', () => {
  ...
  it('never hires anybody straight onto a fortune', () => {
    // Nobody is handed the eight-seat sushi counter on Placement Day: a fair
    // deals the bottom rung, and the bottom rung earns like one.
    for (const career of [...entryRungs(BASIC_CAREERS), ...entryRungs(GRADUATE_CAREERS)]) {
      expect(career.salary).toBeGreaterThanOrEqual(2_000_000)
      expect(career.salary).toBeLessThanOrEqual(7_000_000)
    }
  })
  ...
  it('keeps the graduate pool narrow: a dependable wage, never a spectacular one', () => {
    // The shūkatsu prize is a floor, not a ceiling: a whole graduate life fits
    // in a band a single trade ladder climbs through twice over.
    expect(spread(GRADUATE_CAREERS)).toBeLessThan(3_000_000)
    expect(Math.min(...GRADUATE_CAREERS.map((c) => c.salary))).toBeGreaterThanOrEqual(5_000_000)
  })
  ...
  it('keeps the gap in the means small enough for the board to answer', () => {
    expect(mean(GRADUATE_CAREERS) - mean(BASIC_CAREERS)).toBeLessThan(3_000_000)
  })
  ...
  it('keeps the professions that could not possibly vary on a flat salary', () => {
    for (const id of ['career-jp-hospital-surgeon', 'career-jp-university-professor', 'career-jp-ministry-section-chief']) {
      expect(findCareer(id, EDITION_JAPAN)?.payPerPip).toBeUndefined()
    }
  })

  it('pays the trades whose weeks really do differ by the spin', () => {
    // The stall's queue, the courier's peak season, the artist's reader survey.
    for (const id of ['career-jp-ramen-stall-owner', 'career-jp-parcel-courier', 'career-jp-manga-artist']) {
      expect(findCareer(id, EDITION_JAPAN)?.payPerPip).toBeGreaterThan(0)
    }
  })
