 ...
 *
 * The route mirrors the USA skeleton at ×100 (`edition.test.ts` pins that
 * mechanically) and the CPU is proven scale-invariant, so in principle every
 * measured USA property carries over with two more zeros. In practice one
 * thing genuinely differs — the house catalogue's depreciation tilt — and a
 * principle nobody measures is a hope. So this suite plays the same seeded
 * games the USA suite plays, on the Japan board, and asserts the same bands in
 * yen: the even opening fork, Straight to Work as the volatile road, the
 * playable economy at every difficulty, and a computer that still sometimes
 * chooses a house full of noise.
 *
...
/** Drives a complete Japan game, taking the option at `optionBias % length`. */
const playGame = (
  seed: number,
  playerCount: number,
  optionBias: number,
  ...
  expect(finalState.editionId).toBe('japan')
...
describe('every japan game reaches a conclusion', () => {
...
describe('the yen economy stays in a playable band', () => {
  ...
  it('keeps normal comfortably profitable — the USA band, in yen', () => {
    ...
    // Measured before the die: mean ¥65.9M, median ¥64.4M over these 60 games — the USA
    // board's band ×100, a touch leaner for the house tilt, and nobody ever
    // bust on normal.
    expect(mean(normal.totals)).toBeGreaterThan(40_000_000)
    expect(mean(normal.totals)).toBeLessThan(70_000_000)
    expect(median(normal.totals)).toBeGreaterThan(40_000_000)
...
describe('neither opening lane is the right answer, in yen either', () => {
  ...
  it('splits the wins between the two lanes', () => {
    // Measured at 57.5% over these 240 games, against 53.8% while the longer
    // boards existed. The band is the wider one this file already held the
    // fifteen-minute board to: every fork on the route is the wheel's own call
    // now, and the shortest session has the fewest turns for the mid-career and
    // later forks' own roulette to average back out.
  ...
  it('does not let either opening lane run away with all the volatility', () => {
    ...
    const ratio = spread(sample.work) / spread(sample.university)
    expect(ratio).toBeGreaterThan(0.7)
    expect(ratio).toBeLessThan(1.4)
  })

  it('leaves neither lane the obvious money play', () => {
    const gap = Math.abs(mean(sample.university) - mean(sample.work))
...
describe('the computer can play the japan board unaided', () => {
  ...
  it('still buys houses on a board where half the ladder depreciates', () => {
    // The depreciation tilt must make the farmhouse an argument, not make the
    // computer a lifelong renter.
    const owners = games.flatMap((g) => g.players).filter((p) => p.house !== null)
    expect(owners.length).toBeGreaterThan(0)
  })
})

describe('insurance pays off, rarely but really, in yen', () => {
